...
@st.cache_data(ttl=1800)
def get_batter_pitcher_matchup(player_id, pitcher_id, start_date, end_date):
    """Return a heavily shrunk batter-vs-pitcher HR signal."""
    if not player_id or not pitcher_id:
        return {"pa": 0, "hr": 0, "hr_pa": LEAGUE_HR_PA}
    try:
        df = statcast_batter(start_date, end_date, player_id)
        if df is None or df.empty or "pitcher" not in df.columns:
            return {"pa": 0, "hr": 0, "hr_pa": LEAGUE_HR_PA}
        df = df[df["pitcher"].eq(int(pitcher_id))].copy()
    ...
    for _, game in games_df.iterrows():
        for side in ("away", "home"):
            ...
            for _, hitter in hitters.iterrows():
                pid = int(hitter["id"])
                b30 = get_batter_window(pid, start_30.isoformat(), selected_date.isoformat())
                b90 = get_batter_window(pid, start_90.isoformat(), selected_date.isoformat())
                matchup = get_batter_pitcher_matchup(pid, pitcher_id, start_90.isoformat(), selected_date.isoformat())
...
st.caption("MLB HR Predictor v7 • Mobile cards • expanded explanations • normalized scoring")
