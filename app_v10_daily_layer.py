...
def get_daily_statcast_layer(start_date, end_date, player_type):
    """Build one cached all-MLB Statcast layer for the selected recent window."""
    frames = []
    for a, b in _five_day_ranges(start_date, end_date):
        try:
            df = savant_csv(a, b, player_type)
...
st.caption("⚡ v10 Daily Data Layer: one cached league-wide recent Statcast download + compact season baselines. Change settings, then press Run to refresh predictions.")
st.divider()
...
if run:
    ...
    start_recent = selected_date - timedelta(days=recent_days)
    start_long = selected_date - timedelta(days=90)
    end_date = selected_date.isoformat()

    # ---------------------------------------------------------
    # FAST PIPELINE
    # 1) Fetch MLB metadata concurrently.
    # 2) Build the candidate pool locally.
    # 3) Prefetch all unique pitchers/hitters concurrently.
    # 4) Calculate every model feature locally.
    # ---------------------------------------------------------
    progress = st.progress(0, text="⚡ Preparing today's MLB slate…")

    game_records = games_df.to_dict("records")
    lineup_jobs = [(int(g["game_pk"]), side) for g in game_records for side in ("away", "home")]
    lineup_map = parallel_map(
        lineup_jobs,
        lambda key: get_lineup(key[0], key[1]),
        workers=8,
    )
    progress.progress(0.15, text="⚡ MLB lineups loaded — building candidate pool…")

    roster_jobs = [(int(g[f"{side}_id"]), season) for g in game_records for side in ("away", "home")]
    ...
    for g in game_records:
        ...
        unique_weather[game_pk] = (g.get("venue_lat"), g.get("venue_lon"), end_date, g.get("game_time"))
        for side in ("away", "home"):
            ...
            if pitcher_id:
                unique_pitchers[int(pitcher_id)] = pitcher_name
            ...
            if confirmed:
                ...
                # Pending lineups: 8 likely hitters is enough for the fast preview.
                hitters = hitters.head(8).copy()

            for _, h in hitters.iterrows():
                candidate_specs.append({
                    "game": g, "side": side, "team": team, "team_id": team_id,
                    "opponent": opponent, "pitcher_id": int(pitcher_id) if pitcher_id else None,
                    "pitcher_name": pitcher_name, "confirmed": confirmed,
                    "pid": int(h["id"]), "player": h["name"],
                    "order": h["order"],
                })

    unique_hitter_ids = sorted({c["pid"] for c in candidate_specs})
    unique_pitcher_ids = sorted(unique_pitchers)

    progress.progress(0.25, text="⚡ Loading the daily Statcast data layer…")

    # ONE cached league-wide recent hitter dataset, split into <=5-day chunks.
    recent_layer = get_daily_statcast_layer(
        start_recent.isoformat(), end_date, "batter"
    )
    progress.progress(0.55, text="⚡ Recent hitter data loaded — loading season baselines…")

    season_batter_map = build_season_batter_map(season)
    season_pitcher_map = build_pitcher_map(season)

    # Reuse the same recent all-MLB layer for pitcher tendency + BvP matchup.
    progress.progress(0.82, text="⚡ Aggregating pitcher + matchup data locally…")
    ...
    for c in candidate_specs:
        g = c["game"]
        b_recent = summarize_recent_all(recent_layer, c["pid"], start_recent)
        b_long = season_batter_map.get(c["pid"], {
            "pa": 0, "hr": 0, "hr_pa": LEAGUE_HR_PA,
            "barrel_rate": DEFAULT_BARREL, "hard_hit_rate": DEFAULT_HARD_HIT,
            "ev": DEFAULT_EV, "bbe": 0,
        })
        ...
        if c["pitcher_id"]:
            p90 = season_pitcher_map.get(c["pitcher_id"], {"hr_rate": LEAGUE_HR_PA, "hr": 0, "pa": 0})
            matchup = summarize_recent_pitcher(recent_layer, c["pitcher_id"], c["pid"])
        else:
            p90 = {"hr_rate": LEAGUE_HR_PA, "hr": 0, "pa": 0}
            matchup = {"hr_rate": LEAGUE_HR_PA, "hr": 0, "pa": 0}
        matchup["hr_pa"] = matchup.get("hr_rate", LEAGUE_HR_PA)
        weather = weather_map.get(int(g["game_pk"]), {"condition": "Unknown", "temp_f": np.nan, "wind_mph": np.nan, "multiplier": 1.0})

        result = model_player(
            b_recent, b_long, p90, c["confirmed"],
            c["order"] if pd.notna(c["order"]) else 5,
            matchup=matchup, weather=weather,
        )

        rows.append({
            "Player": c["player"], "Team": c["team"], "Opponent": c["opponent"],
            "Opposing SP": c["pitcher_name"], "Venue": g["venue"],
            "Game PK": int(g["game_pk"]), "Player ID": c["pid"],
            "Weather": weather.get("condition", "Unknown"),
            "Temp F": weather.get("temp_f"), "Wind MPH": weather.get("wind_mph"),
            "Matchup PA": matchup.get("pa", 0), "Matchup HR/PA": matchup.get("hr_pa", LEAGUE_HR_PA),
            "HR Probability": result["probability"], "Score": result["score"],
            "Confidence": result["confidence"], "HR/PA": b_recent["hr_pa"],
            "Barrel %": b_recent["barrel_rate"], "Hard-Hit %": b_recent["hard_hit_rate"],
            "Avg EV": b_recent["ev"], "Pitcher HR/PA": p90["hr_rate"],
            ...
            "Reason": "", "Lineup Penalty": result["lineup_penalty"],
            "Data Confidence": result["data_confidence"], "Pick Type": "",
        })

    progress.progress(1.0, text="✅ Predictions ready")
    ...
    result_df = result_df.sort_values(
        ["HR Probability", "Score", "Data Confidence"], ascending=False
    ...
    # Keep a compact history of the current top 10.
    for rank, (_, h) in enumerate(result_df.head(10).iterrows(), start=1):
        st.session_state.setdefault("prediction_history", []).append({
            "date": selected_date.isoformat(), "game_pk": int(h["Game PK"]),
            "player_id": int(h["Player ID"]), "player": h["Player"],
            "rank": rank, "probability": float(h["HR Probability"]),
        })
...
st.caption("MLB HR Predictor v10 • Daily Statcast Data Layer • Data: MLB StatsAPI + Baseball Savant/Statcast via pybaseball")
