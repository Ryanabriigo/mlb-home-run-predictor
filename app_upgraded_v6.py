...
if run:
    ...
    for _, game in games_df.iterrows():
        for side in ("away", "home"):
            ...
            for _, hitter in hitters.iterrows():
                ...
                matchup = get_batter_pitcher_matchup(
                    pid,
                    pitcher_id,
                    start_90.isoformat(),
                    selected_date.isoformat(),
    ...
    hist = evaluate_history()
    if hist is not None and not hist.empty:
        checked = hist[hist["HR Actual"].notna()].copy()
        if not checked.empty:
            top5 = checked[checked["rank"] <= 5]
            top1 = checked[checked["rank"] == 1]
            m1, m2, m3 = st.columns(3)
            with m1:
                st.metric("Top Pick HR Rate", f'{top1["HR Actual"].gt(0).mean():.1%}' if not top1.empty else "—")
            with m2:
                st.metric("Top 5 HR Rate", f'{top5["HR Actual"].gt(0).mean():.1%}' if not top5.empty else "—")
            with m3:
            ...
            st.dataframe(
                checked[["date","rank","player","probability","HR Actual"]]
...
st.caption("MLB HR Predictor v5 • Data: MLB StatsAPI + Baseball Savant/Statcast via pybaseball")
